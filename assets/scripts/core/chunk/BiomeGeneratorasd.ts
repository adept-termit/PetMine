...
@ccclass('BiomeGeneratorasd')
export class BiomeGeneratorasd extends Component {
    ...
    private getBlockAtPosition(x: number, y: number, z: number): number {
        if (
            ...
            // //первой генериции, пустона внутри
             //return 1

            //для регенирации чанка
    ...
    private _onKeyDown(event: EventMouse) {
        if (event.getButton() === EventMouse.BUTTON_LEFT) {
            ...
            if (PhysicsSystem.instance.raycastClosest(this._ray)) {
                ...
                const localHitPoint = this._block.inverseTransformPoint(new Vec3(), hitPoint);
            ...
            //
            //         // Определяем координаты блока внутри чанка
            //         const blockX = Math.floor((hitPoint.x - this.chunkPosSpawn.x));
            //         const blockY = Math.floor((hitPoint.y - this.chunkPosSpawn.y));
            //         const blockZ = Math.floor((hitPoint.z - this.chunkPosSpawn.z));
            //
            //
            //         // Удаляем блок из данных чанка
            //         const blockIndex = this._chunkData.blocks.findIndex(block =>
            //             block.x === blockX && block.y === blockY && block.z === blockZ
            //         );
            //
            //         if (blockIndex !== -1) {
            //             this._chunkData.blocks.splice(blockIndex, 1); // Удаляем блок
            //         }
    ...
    // Функция для перегенерации чанка
        ...
        for (let y = 0; y < this.chunkSizeHeight; y++) {
            for (let x = 0; x < this.chunkSizeWidth; x++) {
                for (let z = 0; z < this.chunkSizeWidth; z++) {
                    ...
                    if (localPositionInChunk.x === pos.x &&
                        localPositionInChunk.y === pos.y &&
                        localPositionInChunk.z === pos.z
                    ) {
                        continue;
                    }

                    this._chunkData.blocks.push(new Vec3(x, y, z))
                    this.generateBlock(x, y, z, 0);
                }
            }
        }
